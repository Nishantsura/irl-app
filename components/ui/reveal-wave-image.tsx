...
const fragmentShader = `
  ...
  void main() {
    ...
    float flashlight = (1.0 - smoothstep(innerRadius, outerRadius, revealDist)) * uMouseActive;
    float revealAmount = max(uBaseReveal, flashlight);
...
function ImagePlane({
  ...
  useFrame((state) => {
    ...
    // cover-fit scale — computed live so it's always correct
    const vp = state.viewport;
    if (vp.width > 0 && vp.height > 0) {
      const viewAspect = vp.width / vp.height;
      if (aspectRatio > viewAspect) {
        const sy = vp.height / 2;
        meshRef.current.scale.set(sy * aspectRatio, sy, 1);
      } else {
        const sx = vp.width / 2;
        meshRef.current.scale.set(sx, sx / aspectRatio, 1);
      }
    }
